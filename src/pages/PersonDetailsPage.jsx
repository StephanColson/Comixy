...
export function PersonDetailsPage(props) {
    const {person, editions, comicContributors, roles, onEditEdition, onSelectCompendium, onSelectPublisher, organizations, compendium} = props;

    const personContributions = comicContributors?.filter(cc => cc.peopleID === person.id) ?? [];
    const uniqueEditionIDs = [...new Set(personContributions.map(cc => cc.editionID))];
    ...
    const personEditions = uniqueEditionIDs
        .map(editionID => {
            ...
            const publisher = organizations?.find(org => org.id === edition.organizationID);
